...
async function reloginSteamAccount() {
    rl.question("Steam Login Username: ", function(accountName) {
        ...
        db.get('SELECT password,id, cookies, token FROM steamprofiles WHERE username = ?', [accountName], function(err, row) {
          if (err) {
            console.log(err.message);
            process.exit();
          }
          ...
          community.oAuthToken = row.token;
      	  ...
      	  doLogin(accountName, row.password);
        });
    });
}
...
async function reloginSteamAccount() {
    ...
    // loop through all steam accounts
    db.each('SELECT username, password, id, cookies, token FROM steamprofiles', [], function(err, row) {
        if (err) {
            console.log(err.message);
            process.exit();
        }
        ...
        doLogin(row.username, row.password);
    });
...
function doLogin(accountName, password, authCode, twoFactorCode, captcha) {
	community.login({
		...
		console.log(accountName + " Logged on!");

        steamId64 = community.steamID.getSteamID64();
        db.run(`INSERT OR REPLACE INTO steamprofiles (username, steamId, cookies, token, password) VALUES (?, ?, ?, ?, ?)`, [accountName, steamId64, JSON.stringify(cookies), oauthToken, password], function(err) {
          if (err) {
            console.log(err.message);
            process.exit();
          }
